...
// DecomposeResult is the model's proposed changeset: always a decomposition
// (#42) — there is no clarification round-trip, so Subtasks and
// Assumptions are always present, non-nullable arrays, not a discriminated
// union.
type DecomposeResult struct {
	Subtasks    []domain.ProposedTask `json:"subtasks"`
	Assumptions []string              `json:"assumptions" jsonschema:"description=Specific choices made on the user's behalf because they were unspecified (e.g. 'SQLite via mattn/go-sqlite3 for local storage; no sync or server component')."`
}
...
Always return 3-7 subtasks, plus assumptions: every choice you made that the user didn't specify. Be specific — "Booking a mid-size venue for about 30 people," not "assuming a standard venue." Disclose what you deliberately left out, not only what you put in. "No", "none", "no preference", and "out of scope" are decisions, not missing information — record the resulting choice as an assumption.

You do not ask questions. Even if the task is barely specified, decompose the smallest defensible interpretation of it and say so plainly in assumptions — a large or vague assumptions list is itself the signal to the user, and it costs them one glance instead of a round trip.
...
You do not ask questions. Even if what's being built is barely specified, decompose the smallest defensible interpretation of it and say so plainly in assumptions — a large or vague assumptions list is itself the signal to the user, and it costs them one glance instead of a round trip.
...
// buildUserMessage renders the task to decompose.
...
// variable content (the task) last (architecture.md §6).
//
